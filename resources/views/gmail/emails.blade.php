        ...
        <nav class="nav nav-pills nav-fill" id="folderTabs">
            @foreach($folders as $key => $folder)
                ...
                <a class="nav-link {{ $key === 0 ? 'active' : '' }}" 
                   id="{{ Illuminate\Support\Str::slug($folder['name']) }}-tab" 
                   data-toggle="tab" 
                   href="#{{ Illuminate\Support\Str::slug($folder['name']) }}-content" 
                   role="tab" 
        ...
        <div class="tab-content" id="folderContent">
            @foreach($folders as $key => $folder)
                <div class="tab-pane fade {{ $key === 0 ? 'show active' : '' }}" 
                     id="{{ Illuminate\Support\Str::slug($folder['name']) }}-content" 
                     role="tabpanel" 
                     ...
                     <ul>
                         @foreach($messages[$folder['name']] as $message)
                             <li>{{ $message['subject'] }} - {{ $message['from'] }} - {{ $message['date'] }}</li>
                         @endforeach
                     </ul>
